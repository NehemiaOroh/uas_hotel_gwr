<x-admin-layout>
    <div class="py-6 px-4 max-w-lg mx-auto bg-white shadow-md rounded-lg">
        <h1 class="text-2xl font-bold mb-4">Add Booking</h1>
        ...
        <form action="{{ route('bookings.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="room_id" name="room_id" class="w-full mt-1 border-gray-300 rounded-md">
                    ...
                    <option value="{{ $room->room_id }}">{{ $room->room_type }} (Room {{ $room->room_number }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="check_in_date" name="check_in_date" class="w-full mt-1 border-gray-300 rounded-md" required>
            </div>

            <div class="mb-4">
                <label for="check_out_date" class="block text-gray-700 font-medium">Check-Out Date</label>
                <input type="date" id="check_out_date" name="check_out_date" class="w-full mt-1 border-gray-300 rounded-md" required>
            </div>

            <div class="mb-4">
                <label for="total_price" class="block text-gray-700 font-medium">Total Price</label>
                <input type="number" step="0.01" id="total_price" name="total_price" class="w-full mt-1 border-gray-300 rounded-md" required>
            </div>
