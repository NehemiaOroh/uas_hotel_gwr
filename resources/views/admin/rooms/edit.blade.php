    ...
    <div class="py-6 px-4 max-w-lg mx-auto bg-white shadow-md rounded-lg">
        ...
        <form action="{{ route('rooms.update', $room->room_id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="room_number" name="room_number" class="w-full mt-1 border-gray-300 rounded-md" value="{{ $room->room_number }}" required>
            </div>

            <div class="mb-4">
                <label for="room_type" class="block text-gray-700 font-medium">Room Type</label>
                <input type="text" id="room_type" name="room_type" class="w-full mt-1 border-gray-300 rounded-md" value="{{ $room->room_type }}" required>
            </div>

            <div class="mb-4">
                <label for="price_per_night" class="block text-gray-700 font-medium">Price Per Night</label>
                <input type="number" step="0.01" id="price_per_night" name="price_per_night" class="w-full mt-1 border-gray-300 rounded-md" value="{{ $room->price_per_night }}" required>
            </div>

            <div class="mb-4">
                <label for="max_occupancy" class="block text-gray-700 font-medium">Max Occupancy</label>
                <input type="number" id="max_occupancy" name="max_occupancy" class="w-full mt-1 border-gray-300 rounded-md" value="{{ $room->max_occupancy }}" required>
            </div>
